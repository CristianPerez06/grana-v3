...
- [ ] 8.5 **QA nativo CORRIDO** (13/14 en verde, el caso 11 aceptado por excepción escrita en `docs/qa-savings-nativo.md`): esa mitad de la compuerta está levantada. Sigue el **no mergear** con la fase 1 y 2 solas: `feature/add-savings-set-aside` es la branch de integración del modelo y sube con las fases que lo completan. Una branch por fase se probó y se descartó: con merge squash a `main` todo colapsa en un commit igual, así que la branch extra no compraba nada y sí partía el trabajo en dos lugares
