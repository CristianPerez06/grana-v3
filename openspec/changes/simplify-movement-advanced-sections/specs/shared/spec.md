...
El control de reparto SHALL ofrecer los repartos frecuentes como **presets de un gesto** —el pagador 100%, mitad y mitad, y **el otro miembro 100%** (preset "El otro")— más un disparador **"Otro %"** que revela el editor de **porcentaje libre** (`1..99`). El caso "lo pagué yo pero es 100% del otro" SHALL alcanzarse mediante el preset "El otro" (rotulado con el nombre del otro miembro), que fija el split en `{pagador: 0, otro: 100}`: NO SHALL existir un toggle dedicado aparte para ese caso. Los presets y el editor de porcentaje libre SHALL estar disponibles tanto en el alta como en la edición. La presentación mobile de este control (presets + "Otro %") y su paridad entre web-mobile y nativo la fija el requirement «El despliegue de las secciones avanzadas es de superficie mínima y paritario entre las superficies mobile» de la capability `transactions`.

El split **por defecto del hogar** NO forma parte de esta relajación: su editor SHALL seguir acotado a `1..99` (el 0/100 es una decisión por-gasto, no la norma del hogar) y NO SHALL exponer el preset "El otro".
...
- **WHEN** un usuario registra un gasto compartido y toca el preset "El otro"
...
- **AND** no se ofrece el preset "El otro" (0/100) en esa superficie
