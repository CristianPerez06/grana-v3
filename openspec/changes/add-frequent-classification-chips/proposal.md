...
- **Defaults para el usuario nuevo; silencioso solo si no hay nada resoluble.** Con historial, los chips salen de él. Sin historial todavía (usuario nuevo, o un tipo sin movimientos), el formulario ofrece un set de **defaults** (categorías semilla del sistema: Comida › Supermercado, Entretenimiento › Salidas, Transporte para gasto; Sueldo/Freelance/Otros ingresos para ingreso) para que el acelerador sirva desde el primer uso. Recién si ni el historial ni los defaults resuelven nada, no se muestran chips. Nunca clasifica en silencio: el chip es una sugerencia de un tap, siempre visible y editable.
