...
- Una **sección "Tarjeta · a pagar"**: su monto SHALL usar la MISMA definición de "A pagar" del módulo Tarjetas = suma de consumos `pending` menos los reintegros recibidos imputados, de los resúmenes **cerrados/vencidos impagos** (NO el resumen **en curso**, NO los resúmenes que vencen más adelante). La sección SHALL listar los **3-4 consumos de mayor monto** de esos resúmenes (fecha, descripción, monto) y un enlace "ver más" cuando hay más.
...
#### Scenario: El monto de tarjeta coincide con "A pagar" del módulo Tarjetas y excluye en curso y futuros

- **WHEN** el usuario tiene resúmenes cerrados/vencidos impagos por ARS $419.840, un resumen en curso acumulando ARS $120.000 y cuotas que vencen en meses posteriores
- **THEN** la sección "Tarjeta · a pagar" muestra `$419.840` (igual que el header del módulo Tarjetas)
- **AND** NO incluye el resumen en curso ni los resúmenes que vencen más adelante
