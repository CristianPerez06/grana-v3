...
El sistema SHALL proveer un catálogo de instituciones financieras argentinas (`institutions`) pre-cargado con al menos 28 entidades del mercado local. Cada institución tiene nombre, slug único, color de marca, y tipo de ícono. Las instituciones son inmutables para los usuarios.
...
- **THEN** el sistema retorna todas las instituciones con `is_active = true`, ordenadas alfabéticamente
...
El sistema SHALL proveer un catálogo de redes de tarjeta de crédito (`card_networks`) pre-cargado con las redes operativas en Argentina (Visa, Mastercard, Amex, Cabal, Naranja, Naranja X, Maestro). Las redes son inmutables para los usuarios.

Una tarjeta de crédito MUST identificar su red mediante exactamente uno de: `network_id` (FK a `card_networks`) O `other_network_name` (texto libre para redes fuera del catálogo). Nunca ambos, nunca ninguno.
