...
- [x] 8.5 **Las dos mitades de la compuerta, cumplidas.** QA nativo CORRIDO (13/14 en verde, el caso 11 aceptado por excepción escrita en `docs/qa-savings-nativo.md`): esa mitad está levantada. Y la otra también: la branch ya no sube con las fases 1 y 2 solas —lleva además la extracción del módulo—, así que el **no mergear** con la fase 1 y 2 solas: `feature/add-savings-set-aside` es la branch de integración del modelo y sube con las fases que lo completan. Una branch por fase se probó y se descartó: con merge squash a `main` todo colapsa en un commit igual, así que la branch extra no compraba nada y sí partía el trabajo en dos lugares
