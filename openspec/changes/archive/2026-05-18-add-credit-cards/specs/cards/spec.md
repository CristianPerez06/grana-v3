...
El sistema SHALL permitir editar `end_date` y `due_date` de un `card_periods` cuyo estado derivado sea `open`, `closed` u `overdue` (es decir, sin `period_payment`). El sistema SHALL rechazar cualquier intento de editar las fechas de un período `paid`. Si la edición implicaría reubicar transacciones existentes:

- A períodos existentes → mostrar preview al usuario y aplicar al confirmar.
- A períodos que no existen → rechazar con error explicativo.
...
#### Scenario: Edición que reubica a un período inexistente es rechazada

- **WHEN** un usuario intenta extender el `end_date` de un período tal que algunas transacciones imputadas caerían fuera de cualquier período existente
- **THEN** el sistema rechaza el cambio con el mensaje "Para aplicar este cambio, primero configurá las fechas del período siguiente"
- **AND** no se actualiza ninguna fila
...
#### Scenario: Revertir un pago muestra nuevamente el período con deuda

- **WHEN** el usuario revierte el pago de P1 (que había creado P2 vacío) y P2 sigue existiendo
- **THEN** el detalle de la tarjeta muestra P1 como período activo
- **AND** P2 (vacío) aparece en la sección "Próximos resúmenes", no como período activo
