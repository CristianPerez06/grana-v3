...
- [x] 1.2 Renderizar `contextLine` como un `<p>` atenuado con los segmentos separados por `·` y un único caption `common.not_editable` al final. El monto no entra en la línea.
- [x] 1.2b Cuando `getEditableFields` bloquea el monto, dibujar el héroe **read-only** (misma card, mismo tamaño, sin input ni calculadora, moneda como chip estático, `no editable` al pie) en vez de omitirlo.
...
- [x] 2.2 Renderizar la línea y moverla de arriba del héroe a **abajo**, igualando el orden de web.
- [x] 2.2b Mismo héroe read-only para el monto bloqueado.
...
- [ ] 3.4 **Consumo de tarjeta pagado**: el monto se ve en el héroe read-only (card, número grande, `no editable` al pie), y la línea debajo trae tipo · moneda · cuenta · fecha.
