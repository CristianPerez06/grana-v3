...
- [x] 1.3 Ambas superficies consumen la **misma** familia `shared.split.*` (web-mobile vía `tShared('split.*')`, nativo vía `t('shared.split.*')`) — no hay dos juegos paralelos.
...
- [x] 7.5 Nativo revisado y QA aprobado por el tech lead en la app mobile.

## 8. Cierre

- [x] 8.1 Archivar el change (mover a `openspec/changes/archive/2026-08-18-simplify-movement-advanced-sections/`, integrar deltas en `transactions/spec.md` y `shared/spec.md`, `pnpm openspec:check`). Web-mobile verificado por el PO y nativo aprobado por el tech lead.
