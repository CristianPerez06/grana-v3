...
- La salida del hogar es una secuencia app-side en `leaveHouseholdCore` (`@grana/shared`): validaciones, borrado de la membresía, desactivación del hogar si queda vacío.
...
`leaveHouseholdCore` gana un paso previo, antes de borrar la membresía: el RPC `detach_household_classifications(p_household_id)` (`SECURITY INVOKER`, corre bajo el RLS del que sale, que todavía es miembro). Para cada categoría del hogar referenciada por movimientos, reglas o instancias del usuario **no compartidos**, crea una copia propia (mismo nombre, ícono, color, tipo y estado; `canonical_name` con sufijo `-hogar` si ya existe una propia igual), copia bajo ella las subcategorías del hogar que el usuario referencia, y repunta esas filas a las copias. Una subcategoría del hogar bajo una categoría del sistema se copia como propia bajo la misma categoría del sistema (su `canonical_name` lleva sufijo porque la unicidad de subcategorías es por categoría, no por dueño). Los compartidos no se tocan: el invariante de la decisión anterior exige que sigan apuntando a lo del hogar. Todo ocurre en una transacción: si falla, no queda ni una copia a medias ni un movimiento apuntando a algo ilegible. Alternativa descartada: hacer la secuencia app-side con varias llamadas PostgREST; no es atómica y no se puede cubrir con el harness PGlite, que es donde se prueba el resto de la migración.
...
- Resuelta: el sufijo de `canonical_name` al copiar es `-hogar` (y `-hogar-2`, `-hogar-3`… si sigue colisionando). No cambia specs ni tareas.
