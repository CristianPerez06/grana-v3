...
### D5 — `payCardPeriod` es la pieza grande; tests con la reversa
`payCardPeriod` (legs de pago, marca de período pagado, manejo USD subordinado, la reversa de pago, y el **impuesto de sellos** por resumen) es la mutación de mayor riesgo. Se mueve entera con su reversa y viaja con tests que cubran pago simple, pago con USD subordinado, pago con sello, y reversa. Si en la implementación resulta inmanejable como una sola pieza, se permite splitear este change dejando `payCardPeriod` en su propio commit/change — pero el default es un único change "cards mutations".
