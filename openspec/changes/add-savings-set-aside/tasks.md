...
- [ ] 9.7 **La branch no se mergea con la fase 1 sola.** Decisión del producto, no del change: la fase 1 sube a producción junto con las fases que la completan. Sola le da al usuario un *disponible* real pero ningún lugar donde decir **para qué** guardó, que es la mitad de la idea. Las fases siguientes se construyen sobre esta branch. Consecuencia operativa: la branch va a vivir varias fases, así que **traer `main` con `git merge` cada vez que se archive un change ahí** — hoy ya son 4 commits de diferencia, y la deuda de integración crece sola. `AGENTS.md` pide el archivado en la branch antes del merge: eso sigue valiendo, solo que el merge llega más tarde
