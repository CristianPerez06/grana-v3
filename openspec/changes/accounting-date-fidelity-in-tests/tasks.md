...
- [ ] 1.1 En `packages/recurrences/__tests__/support/pglite-postgrest.ts`, hacer que
...
- [ ] 1.2 Borrar la conversión con getters locales (`getFullYear`/`getMonth`/`getDate`)
...
- [ ] 1.3 Agregar un test del propio andamio que fije la fidelidad: una columna `date`
...
- [ ] 2.1 `TZ=UTC pnpm --filter @grana/recurrences test` → 150 pasan, 0 fallan.
- [ ] 2.2 `TZ=America/Argentina/Buenos_Aires pnpm --filter @grana/recurrences test` →
...
- [ ] 2.3 `pnpm test` completo (los 12 scripts) sin regresiones en los otros paquetes.

## 3. Fijar el huso financiero en los tests

- [ ] 3.1 Prefijar con `TZ=America/Argentina/Buenos_Aires` el script `test` de los 11
...
- [ ] 3.2 Agregar al job `monorepo-health` de `.github/workflows/ci.yml` una aserción
...
- [ ] 3.3 Verificar la aserción por los dos lados: falla si se le quita el prefijo a un
      paquete, pasa con los 12 puestos.

## 4. Dejar la regla escrita donde se busca

- [ ] 4.1 Documentar en `AGENTS.md` que los tests corren en la zona horaria financiera
      ...
      nueva.
- [ ] 4.2 `pnpm openspec:check` en verde.
