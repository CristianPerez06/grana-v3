...
Agrupa las preferencias de visualización y la administración personal del usuario bajo la ruta `/settings`, con las mismas secciones y en el mismo orden en web y en mobile. Incluye el toggle de mostrar/ocultar centavos en todos los importes (persistido por cookie de 1 año en web, SecureStore en mobile, sin afectar los cálculos), el selector de idioma, el acceso a la gestión de categorías personalizadas y subcategorías (las categorías de sistema se ven pero no se editan), y la sección Seguridad, que aloja las acciones de cuenta que necesitan formulario propio — hoy el cambio de contraseña, en la ruta hija `/settings/password`.

No muta el ledger ni la lógica de cálculo. Las secciones de preferencias son UI-only y de aplicación inmediata; la sección Seguridad es la excepción deliberada: sus filas no mutan nada en el lugar, navegan a una ruta hija donde vive la acción (el flujo del cambio de contraseña en sí lo define la capability `auth`).
## Requirements
