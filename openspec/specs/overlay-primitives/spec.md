# overlay-primitives Specification

## Purpose

Define los primitivos UI de overlay reutilizables para `apps/web` y `apps/mobile`: `Drawer` (panel lateral sobre scrim), `Popover` (contenido anclado a un trigger), `Segmented` (selector segmentado de opción única) y `Switch` (on/off). Cubre los contracts de props compartidos en `@grana/ui-contracts` (`DrawerProps`, `PopoverProps`, `SegmentedProps`/`SegmentedOption`, `SwitchProps`) y las dos implementaciones idiomáticas (HTML/Radix en web, React Native en mobile). Estos primitivos son la base del rediseño del formulario de movimientos como drawer.
...
- **THEN** invoca `onOpenChange(false)`
