...
Define la base del design system de Grana, transversal a web y mobile: las tres capas de componentes (primitivos, compartidos de feature, compuestos de página) con su ubicación canónica por plataforma, la resolución de los custom properties de `@grana/ui-tokens` en ambos runtimes, y las reglas de composición de los dos primitivos base — `Card` para superficies con shell y `Button` para acciones — que se componen en lugar de re-tipear sus clases inline. Responde la pregunta "¿puedo tipear `rounded-2xl border bg-card` inline, o tengo que componer el primitivo?". No cubre los primitivos de overlay (ver `overlay-primitives`), el componente de encabezado de página (ver `page-header`) ni el carveado del repo (ver `repo-architecture`).
## Requirements
