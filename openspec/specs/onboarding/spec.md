...
- **WHEN** un usuario completa `(onboarding)/perfil` en mobile, cierra/mata la app, y la abre de nuevo
- **THEN** el splash gate detecta `onboarding_completed_at IS NULL` y lo redirige a `(onboarding)/welcome`
- **AND** las respuestas previas de `/perfil` siguen reflejadas en el estado del perfil — no se le piden de nuevo

#### Scenario: Usuario navega hacia atrás en el stack a mitad del wizard (mobile)

- **WHEN** un usuario llega a `(onboarding)/saldo-actual` y usa el gesto/botón de back para volver a `(onboarding)/perfil`
...
- **AND** el único elemento accionable es un botón "Empezar" que dispara `router.push('/(onboarding)/perfil')`
- **AND** no se modifica ningún registro en `profiles`, `accounts` ni otras tablas
...
- **WHEN** un usuario tocca la card "Vista simple" en `(onboarding)/perfil` y presiona "Continuar"
- **THEN** el sistema persiste `profiles.mode='novato'` vía `supabase.from('profiles').update(...)`
- **AND** NO crea ninguna fila en `accounts`
- **AND** ejecuta `router.replace('/(onboarding)/saldo-actual')`
...
- **WHEN** un usuario carga `(onboarding)/perfil` en mobile con cualquier modo seleccionado
...
#### Scenario: La pantalla /perfil no muestra ningún botón de saltar (web)
...
#### Scenario: La pantalla /saldo-actual no muestra ningún botón de saltar (web)

- **WHEN** un usuario navega a `/onboarding/saldo-actual`
- **THEN** el formulario NO contiene un botón con texto "Saltar este paso", "Saltar", "Omitir" ni equivalente
- **AND** el único CTA disponible es "Continuar"

#### Scenario: El monto en pesos del primary account es obligatorio en /saldo-actual (web)
...
- **WHEN** un usuario abre `(onboarding)/perfil` en mobile
...
- **WHEN** un usuario abre `(onboarding)/saldo-actual` en mobile
- **THEN** el formulario NO contiene un `Button` o `Pressable` con texto "Saltar este paso", "Saltar", "Omitir" ni equivalente
- **AND** el único CTA disponible es "Continuar"

#### Scenario: El monto en pesos del primary account es obligatorio en saldo-actual (mobile)

- **WHEN** un usuario en `(onboarding)/saldo-actual` deja el `TextInput` de ARS del grupo principal vacío y presiona "Continuar"
...
- **WHEN** un usuario en `(onboarding)/saldo-actual` carga `0` en el `TextInput` ARS del grupo principal y presiona "Continuar"
...
- **WHEN** el INSERT de `account_currencies` falla por cualquier razón después de un INSERT exitoso en `accounts` durante el submit de `(onboarding)/perfil`
