...
| 16 | `savings` | ✅ Done (fase 1) | **Guardar**: apartar parte del disponible y liberarlo, por moneda, **fuera del ledger** (tabla `availability_reserve`, sin `account_id` — una reserva no vive en ninguna cuenta). Guardar NO es un movimiento: no crea filas en `transactions`, no toca ningún saldo y no aparece en Movimientos. El **disponible real** (`saldo de cuentas propias − guardado`) y el **flujo reservado del mes** son lectura normativa en SQL (`get_available_sums`, `get_reserve_flow_sums`, migración `0057`): el Hero, el tope del drawer y la validación del write path los consumen, ninguno recompone la resta. Tope al guardar (el disponible) y piso al liberar (lo reservado); **gastar no tiene tope** y el disponible negativo se muestra tal cual sin achicar la reserva. Superficies: la **fila permanente** de la card de saldo en el mes corriente (cuatro estados: guardaste / liberaste / guardado / guardar algo), un **overlay** con detalle + form (Drawer en web, BottomSheet en nativo, sin ruta ni entrada de menú) y una **tira de sugerencia** post-ingreso sobre `guidance`, máximo una vez por mes, que recuerda el porcentaje y no el importe. Sobres/envelopes quedó **descartado** como modelo. Pendiente: propósitos (fase 2) y posiciones (fase 3). **Superficie propia**: ruta `/savings`, rótulo «Ahorro e inversión» (`extract-savings-module`). El módulo se puede apagar, y la regla es que **la bandera controla la superficie, nunca los números**: con guardado > 0 y el módulo apagado, la fila del dashboard se queda y el módulo entra en estado degradado —se lee y se puede volver a usar, no se agrega ni se reorganiza— porque apagarlo nunca puede secuestrar plata |
