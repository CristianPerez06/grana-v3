...
| 14 | `settings` | ✅ Done | Categorías personalizadas, preferencias de usuario (mostrar centavos, etc.) + paridad mobile (toggle centavos, switcher de idioma reactivo via `LocaleProvider`, CRUD de categorías en mobile). Sección **Seguridad** con cambio de contraseña desde el área autenticada en `/settings/password` (web + mobile): verifica la actual con un cliente descartable y revoca las demás sesiones con `signOut({ scope: 'others' })` sin desloguear la actual |
...
| 16 | `savings` | ✅ Done (fases 1 y 2) | **Guardar**: apartar parte del disponible y liberarlo, por moneda, **fuera del ledger** (tabla `availability_reserve`, sin `account_id` — una reserva no vive en ninguna cuenta). Guardar NO es un movimiento: no crea filas en `transactions`, no toca ningún saldo y no aparece en Movimientos. El **disponible real** (`saldo de cuentas propias − guardado`) y el **flujo reservado del mes** son lectura normativa en SQL (`get_available_sums`, `get_reserve_flow_sums`, migración `0057`): el Hero, el tope del drawer y la validación del write path los consumen, ninguno recompone la resta. Tope al guardar (el disponible) y piso al liberar (lo reservado); **gastar no tiene tope** y el disponible negativo se muestra tal cual sin achicar la reserva. Superficies: la **fila permanente** de la card de saldo en el mes corriente (cuatro estados: guardaste / liberaste / guardado / guardar algo), un **overlay de ACTOS** —sin vista de detalle: abre directo a lo que se tocó (Drawer en web, BottomSheet en nativo) y solo lo monta el módulo— y una **tira de sugerencia** post-ingreso sobre `guidance`, máximo una vez por mes, que recuerda el porcentaje y no el importe. Sobres/envelopes quedó **descartado** como modelo. **Propósitos (fase 2) construidos**: lo guardado se reparte en propósitos con nombre y emblema, con «Sin destino» como resto; destinar y quitar destino NO mueven plata ni cambian el disponible —solo dicen para qué es parte de lo ya guardado— y el piso al volver a usar es **por grupo**, no el total. Pendiente: posiciones (fase 3). **Superficie propia en las DOS apps**: ruta `/savings`, rótulo «Ahorro e inversión» (`extract-savings-module`) — sidebar en web, sheet del menú en nativa. La fila de la card de saldo explica el disponible y **navega**; dejó de operar. El módulo nativo está **implementado y con el QA visual nativo corrido** (13 de 14 casos, en un iPhone 16 Pro real; el caso del teclado sobre el botón de confirmar quedó aceptado por excepción escrita en `docs/qa-savings-nativo.md`). El módulo se puede apagar, y la regla es que **la bandera controla la superficie, nunca los números**: con guardado > 0 y el módulo apagado, la fila del dashboard se queda y el módulo entra en estado degradado —se lee y se puede volver a usar, no se agrega ni se reorganiza— porque apagarlo nunca puede secuestrar plata |
| 17 | `cashflow` | 🔜 Future | Proyecciones de flujo de caja |
| 18 | `investments` | 🔜 Future | Inversiones. **Se construye adentro de `savings`**, no como módulo aparte: son un modelo de datos, no dos (`docs/modelo-de-dinero.md` §1) |
