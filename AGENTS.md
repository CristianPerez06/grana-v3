...
.agents/           # Codex: our own skills and the OpenSpec workflows, both rendered as skills (versioned)
```
...
- **`.claude/` and `.agents/`** — agent instructions, versioned on purpose so a fresh session starts with them. One folder per tool, never two copies of the same file: Claude Code reads `.claude/`, Codex reads `.agents/`. Our own instructions live in `.claude/skills/grana-*/` and in this file, and **every one of them has a Codex copy** in `.agents/skills/grana-*/`. The `/opsx:*` commands have one too, as `.agents/skills/source-command-opsx-*/`: Codex has no slash commands, so a skill is the only shape they can take there. All of it is generated output kept in the repository on purpose — a skill that exists on one laptop only is a skill Codex silently does not have, and nobody finds out by reading a file that isn't there. The `openspec-*` files in both folders are **generated** by `openspec update` — edit them and the next run wipes it.

**The Codex copies go stale, and nothing reports it.** They are generated from `.claude/`, so renaming or splitting a skill leaves the old copy sitting there under its old name, still valid, still loadable, describing something that no longer exists. That is what happened to `grana-explore`: it survived the split into `grana-deep-explore` and `grana-lightweight-explore`, and was only noticed because a change went to commit the folder and found a skill AGENTS.md had stopped naming. When you add, rename or remove anything in `.claude/skills/`, check that `.agents/skills/` matches — `ls` both is enough, and `git status` will show a new copy but never a stale one. Which workflows exist is a machine-level OpenSpec setting, not a repo one, so a fresh clone on another machine regenerates whatever that machine's profile says. This repo expects five workflows and deliberately drops the generated `explore`. Two skills of ours replace it: `/grana-lightweight-explore`, the everyday one, written to follow [§ Talking to the user](#talking-to-the-user); and `/grana-deep-explore`, the generated text copied word for word, so it only runs when typed by name and `openspec update` can't rewrite it. On a new machine, reproduce it before the first `openspec update`:
...
| **Migrations are the schema truth** | No `schema.sql` reference file. The source of truth is the ordered migration files in `supabase/migrations/`. `packages/supabase/src/types.ts` is the TypeScript mirror of that truth, **written by hand** (see the row below) — it is a copy, not a second source: when the two disagree, the migration is right. |
...
| **Supabase is online-only, and the CLI is not part of the loop** | There is no local Supabase instance and there never will be. The database lives in the hosted project; migrations are applied by pasting SQL into that project's SQL Editor in the dashboard. Any task or spec that says "local DB" means the online Supabase project. **The Supabase CLI is not used at all** — no `supabase login`, no `supabase link`, no `supabase gen types`. So `packages/supabase/src/types.ts` is **maintained by hand**: when a migration changes the public contract (a table, a column, an RPC's signature or its return shape), the entry is written to match the SQL that was applied, compared against it line by line, and `pnpm typecheck` + `pnpm typecheck:mobile` are run. Know what that costs: a typecheck only proves the code agrees with the file, never that the file agrees with the remote schema. Nothing compares the whole schema automatically, so a column that changed and was not hand-edited stays wrong and silent — which is why the comparison is per-migration and happens while the SQL is still in front of you. |
...
| 10 | `recurring-movements` | ✅ Done | Recurrence templates and generated instances; confirm, skip, postpone. **How a rule ends is one question with three exclusive answers** (`how-a-recurrence-ends`): sin límite / en una fecha / después de N vencimientos, on all three creation paths and in edit — the pair `(end_date, max_occurrences)` is derived from the chosen answer, never from whatever is left in the other field. `max_occurrences` counts calendar POSITIONS, not `recurrence_instances` rows (a seeded rule has no row for its first occurrence); `recurrence_positions_spent` is the normative count and `recurrence_positions_spent_batch` reads a whole list in one call. **«Finalizada» is DERIVED, never written to `recurrences.status`** — the question is whether the calendar produces anything from today on, evaluated as if a paused rule resumed today. A pause covers the days AFTER `paused_from`, so pausing on the day an occurrence falls cannot unspend it (migration 0071, mirrored by `subtractPauses`). **A rule is named in ONE order, decided in `recurrenceTitle` (`@grana/recurrences`)**: descripción → subcategoría → categoría → etiqueta del tipo, blank counting as absent. Do not retype the chain in a screen — it was retyped nine times and diverged. A surface that names the RULE reads the rule's own fields (`attachRuleClassification` brings them with the pendientes); a surface that names an OCURRENCIA reads its snapshot, which the user can edit one row at a time. **The dashboard's «Compromisos» are the one surface still OUTSIDE that decision** — `@grana/dashboard` builds the name in its query and returns resolved text, using the category's STORED name instead of the translated one, so in English it says «Servicios» where the hub says «Services». Known divergence, written into the spec; closing it means returning the classification instead of the text. **A rule is trabada when it holds two or more unresolved occurrences and the oldest has already come due** — one line per rule, never a total, and the count is «lo que hay para revisar», not the whole backlog. **Un vencimiento se puede resolver ANTES de que su fecha llegue**, de dos formas (`recurrence-link-movement`): «Ya lo pagué» crea el movimiento, y «Ya lo tengo cargado» señala uno que ya existe sin crear nada. Pagar antes NO mueve el calendario. La ocurrencia se escribe ya resuelta —nunca una pendiente con fecha futura, que el bloque de por revisar mostraría—. `resolution_kind` distingue las dos: `created` se deshace borrando el movimiento (#104, fuera de alcance) y `linked` **desvinculando**, que conserva el movimiento y devuelve el vencimiento a por revisar. **Desvincular es todo o nada**: si la vinculación convirtió un gasto personal en compartido, revierte también esa conversión, y si una liquidación vigente lo impide no hace NADA y dice qué resolver primero —revertirla si está completada, cancelarla si está pendiente; nunca «revertí» sobre una pendiente, que es una operación que el sistema no ofrece para ese estado—. **Una posición del límite se gasta al llegar su fecha O al resolverse antes, una sola vez**: los dos conjuntos son disjuntos por construcción y el caminante NO se extiende más allá de hoy, porque la generación lo comparte y empezaría a materializar el futuro. Whether the backlog is still being rebuilt is said ONCE for the screen and attributed to no rule: `remaining` is one number for the whole run and the generator only queries ACTIVE rules, so hanging it off each line makes a paused rule claim a recovery nobody is running |
