...
.agents/           # Codex: the same OpenSpec workflows, rendered as skills (versioned)
```
...
- **`.claude/` and `.agents/`** — agent instructions, versioned on purpose so a fresh session starts with them. One folder per tool, never two copies of the same file: Claude Code reads `.claude/`, Codex reads `.agents/`. Our own instructions live in `.claude/skills/grana-*/` and in this file; the `openspec-*` files in both folders are **generated** by `openspec update` — edit them and the next run wipes it. Which workflows exist is a machine-level OpenSpec setting, not a repo one, so a fresh clone on another machine regenerates whatever that machine's profile says. This repo expects five workflows and deliberately drops `explore` — `/grana-explore` replaces it, because the generated one asks for diagrams and "think deeply" against [§ Talking to the user](#talking-to-the-user). On a new machine, reproduce it before the first `openspec update`:
  ...
  CI is what notices: `pnpm openspec:check` runs `.github/scripts/check-openspec-workflows.mjs`,
  which fails if the generated workflows are not exactly those five — in `.claude/commands/opsx/`,
  in `.agents/skills/`, or in `.claude/skills/`, a location the CLI used to write and no longer
  does. It reads the working tree, so it fails the moment `openspec update` writes the file,
  before it is staged. The rule is the whole set rather than a ban on `explore` so that a sixth
  workflow nobody asked for is caught too. `.gitignore` is deliberately NOT used for this: both
