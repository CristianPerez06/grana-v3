...
1. **`pnpm openspec:validate`** — runs `@fission-ai/openspec@1.13.0 validate --specs --strict`, the same pinned command CI runs. It catches what a grep cannot: requirements missing `SHALL`/`MUST`, requirements with no `#### Scenario:` block, and Purpose blocks that are too brief.
