...
- **Read-only routes** (dashboard, account/card detail, settings views) → **RSC + `<Suspense>` per section**. The route's `page.tsx` composes async server components; each section's data lives in `lib/<feature>/queries.ts` and is awaited inside its container; loading/error states live in the boundary of each section (see spec `route-loading-and-errors`).
- **Interactive routes** (lots of client-side filters / search / toggles that the user mutates without navigating) → **client shell + `<QueryClientProvider>` + `useQuery`**. Filters live in React state (`useReducer` + context, no URL params). Mutations call invalidation helpers from `lib/<feature>/invalidation.ts`; server actions also call `revalidatePath` helpers from `app/_actions/_helpers.ts` so other RSC routes stay fresh on next navigation. `/transactions` is the reference implementation.

The **data layer is one** in both models: queries are pure functions in `lib/<feature>/queries.ts`, agnostic of caller. Server actions in `app/_actions/queries.ts` wrap them for client consumption via TanStack.
