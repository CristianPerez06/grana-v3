-- ═══════════════════════════════════════════════════════════════════════════
-- 0042 — get_movements_page: filtro opcional para ocultar movimientos compartidos
--
-- El módulo global de Movimientos gana un toggle "mostrar compartidos" (ON por
-- defecto, persistido por usuario). Cuando está OFF, el cliente envía
-- `excludeShared: true` y el listado debe traer solo movimientos NO compartidos.
--
-- Se filtra en SQL (no descartando filas en cliente) para que la paginación
-- (limit+1 lookahead / offset) y cualquier conteo derivado sigan siendo
-- correctos. Único cambio respecto de la 0039: el flag `exclude_shared` en el
-- CTE `f` + la condición `and (not f.exclude_shared or not is_shared)` en el
-- join. El resto de la función es idéntico.
-- ═══════════════════════════════════════════════════════════════════════════
