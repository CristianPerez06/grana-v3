-- ═══════════════════════════════════════════════════════════════════════════
-- 0029 — get_movements_page: RPC de la página global de movimientos
--
-- Reemplaza la paginación por chunks de PostgREST + filtrado en JS de
-- getGlobalMovementsPage (web) por una única función SQL que:
--   · aplica TODOS los filtros en SQL (fechas, categoría, subcategoría con el
--     marker '__none__', moneda, cuenta — incluyendo parents con hijos en la
--     cuenta y pagos de resumen vía period_payments —, tipo funcional, texto
--     y rango de montos),
--   · excluye reimbursements no recibidos o cancelados (regla isHistoryRow:
--     un reintegro pendiente es una expectativa, no historia),
--   · resuelve el linked expense del reimbursement en el mismo query (el
--     self-join que PostgREST no puede embeber — reemplaza attachLinkedExpenses),
--   · devuelve limit + 1 filas para derivar hasMore sin un count extra.
--
-- El shape de cada fila replica el TRANSACTION_SELECT de PostgREST
-- (TransactionWithDetails + linked_expense), así toFinancialMovement consume
-- el resultado sin cambios. La derivación de kind/title espeja
-- toFinancialMovement (apps/web/lib/transactions/movements.ts) — los literales
-- de título ('Pago de resumen', 'Transferencia', …) son los mismos que el
-- matching de texto usaba en JS (movementMatchesText).
--
-- SECURITY INVOKER: RLS aplica con los permisos del usuario que llama
-- (spec web-data-access → "RLS aplica dentro de la RPC").
-- ═══════════════════════════════════════════════════════════════════════════
