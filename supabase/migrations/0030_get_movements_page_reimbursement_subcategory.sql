-- ═══════════════════════════════════════════════════════════════════════════
-- 0030 — get_movements_page: subcategoría del reintegro derivada del gasto
--
-- Un reintegro no guarda categoría ni subcategoría propias: ambas se derivan
-- del gasto vinculado (linked_expense). La 0029 ya embebía la CATEGORÍA del
-- gasto en el objeto `linked_expense`, pero no la SUBCATEGORÍA, así que la fila
-- del reintegro en /movimientos nunca mostraba la subcategoría aunque el gasto
-- sí la tuviera. Acá agregamos `linked_expense.subcategory` (mismo shape que el
-- `subcategory` de una transacción normal) para que toFinancialMovement la
-- herede igual que la descripción y la categoría.
--
-- Solo cambia el bloque `linked_expense` + el join a subcategories del gasto;
-- el resto de la función es idéntico a la 0029.
-- ═══════════════════════════════════════════════════════════════════════════
