-- ═══════════════════════════════════════════════════════════════════════════
-- 0057 — get_movements_page: unificar los campos buscables de la búsqueda
--
-- La búsqueda del feed matcheaba menos que el texto que la fila muestra. Dos
-- huecos: la INSTITUCIÓN de la cuenta —que es el texto principal de la cuenta en
-- la fila (`institutionName?.trim() || name`), o sea que el usuario lee "Galicia"
-- y buscaba "Galicia" sin resultado— y la cuenta DESTINO de un `exchange`, que la
-- fila dibuja igual que la de una transferencia pero el match dejaba afuera.
--
-- Cambia una sola cláusula respecto de la 0042: el `concat_ws` del `ilike` suma
-- `sai.name` / `dai.name`, y el destino pierde su `case when calc.kind =
-- 'transfer'`. El `case` no protegía de nada: `da` viene de
-- `transfer_destination_account_id`, que sólo está poblado en `transfer` y
-- `exchange`, y `concat_ws` ya ignora los null. Los joins a `institutions` (sai,
-- dai) ya estaban — la función los usa para armar el payload.
--
-- El escapeo literal del patrón (más abajo, en el CTE `f`) NO se toca: `%` y `_`
-- tipeados por el usuario siguen siendo texto, no comodines.
--
-- ⚠️ El set de campos buscables está declarado en TypeScript, en
-- `packages/transactions/src/filters.ts` (`movementMatchesText`), que es el que
-- sirve a las dos superficies de detalle de cuenta. No hay forma de compartir
-- código entre esa función y este SQL: cambiar qué es buscable es cambiar LOS
-- DOS. El docblock de allá enumera el set y explica qué queda afuera y por qué.
-- ═══════════════════════════════════════════════════════════════════════════
...
with f as (
  select
    ...
    coalesce((p_filters->>'excludeShared')::boolean, false) as exclude_shared
)
select
...
join transactions t
 ...
 -- Toggle "mostrar compartidos" (OFF ⇒ excluye is_shared).
 and (not f.exclude_shared or not coalesce(t.is_shared, false))
