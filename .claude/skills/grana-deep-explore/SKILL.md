---
name: grana-deep-explore
description: Grana's copy of OpenSpec's original explore, word for word — a thinking partner with diagrams and several threads that may write OpenSpec artifacts after an explicit yes. Use only when the user asks for it by name.
user_invocable: true
...
**Input**: The argument after `/grana-deep-explore` is whatever the user wants to think about. Could be:
