...
Codex: [snapshot] → [3 agentes en paralelo] → [verificaciones propias] → [reporte]
```

```
Usuario: /grana-checkpoint cards
Codex: [snapshot acotado al módulo cards] → [3 agentes con foco en cards] → [verificaciones puntuales del módulo] → [reporte centrado en cards]
```
