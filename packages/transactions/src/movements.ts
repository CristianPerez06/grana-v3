...
// discriminated union that a movement row renders, the pure bridge to
// `resolveMovementView` of `@grana/money-logic`, and the DB→movement mappers of
// the global feed (`toFinancialMovement`, `toInitialBalanceMovement`). Shared so
// both web (the `/transactions` feed, the account detail) and mobile (the card
// statement movements pane, the Movimientos tab) render the same shape. The
// card-specific mapper lives in `@grana/cards`.
...
// ── DB → FinancialMovement mappers (global feed) ──────────────────────────────
// Map a `TransactionWithDetails` DB row to the `FinancialMovement` union a row
// renders. Used by the global feed (`getGlobalMovementsPage`) and the account
// detail. Pure — no I/O. The `detail_href` points at the web route; consumers
// that lack that route (mobile) ignore it (non-navigable rows).
