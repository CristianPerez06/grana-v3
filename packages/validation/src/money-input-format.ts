...
// SHARED across web (DOM input) and mobile (RN TextInput) so both surfaces
// group, cap and canonicalize amounts identically — the single source of truth
// for how a typed money amount looks and what it emits upstream.
//
...
// Hard cap on the integer part — an amount can't have more than this many digits
// before the decimal. Ten digits already covers billions; beyond it the hero
// number stops fitting and the value stops being a real amount.
...
// "0" builds the number naturally. The integer part is capped to
// `MAX_INTEGER_DIGITS` and the fractional part to 2 digits.
//
...
export const toCanonical = (raw: string, allowNegative = false): string => {
  ...
  intPart = intPart.slice(0, MAX_INTEGER_DIGITS) // cap the integer length
