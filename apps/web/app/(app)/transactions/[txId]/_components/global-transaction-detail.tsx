...
import type { Household, MovementSharedInfo } from '@grana/shared'
...
type Props = {
   ...
   * tiene dos imputaciones sobre un solo gasto; pagando cada moneda por su cuenta, cada
   * gasto tiene la suya. Nunca se suman entre monedas.
   */
  ...
  /**
   * Cómo quedó atado a la regla: creado por ella o vinculado a mano. Viaja
...
export const GlobalTransactionDetail = ({
  ...
  if (isTransferLike) {
    ...
    tiles.push(
      <TileRecurrence key="rec" summary={recurrence} relation={recurrenceRelation} />,
      <TilePaymentMethod
        key="paid"
    ...
    if (movement.kind === 'card_payment') {
      // Una fila por imputación, cada una en la moneda de la DEUDA que canceló.
      for (const a of paymentAllocations) {
        const isUsd = a.settlesCurrency === 'USD'
        rows.push({
          key: `settles_${a.settlesCurrency}`,
          icon: <Banknote size={16} strokeWidth={2} aria-hidden />,
          label: isUsd ? t('detail.labels.composition_usd') : t('detail.labels.composition_ars'),
          value: (
            <span className="tabular-nums">
              {money(a.settlesAmount, isUsd ? 'USD' : 'ARS')}
            </span>
          ),
        })
      }
