...
const findActiveHref = (pathname: string) => {
  ...
  if (isActive(pathname, '/settings') && (best === null || '/settings'.length > best.length)) {
    best = '/settings'
  }
  return best
}
...
const Sidebar = () => {
  ...
  return (
    <aside
      ...
      className="hidden shrink-0 md:flex md:w-64 md:flex-col md:gap-1 md:m-3 md:rounded-3xl md:border md:border-border-soft md:bg-card md:py-5 md:shadow-sm"
    >
  ...
  return (
    <>
      <Link
        href="/dashboard"
        onClick={onNavigate}
        className="mb-2 flex items-center px-5 py-2 text-2xl font-bold tracking-tight text-navy"
      >
        grana
      </Link>
...
const SidebarLink = ({
  ...
  <Link
    ...
    className={`flex items-center gap-3 rounded-[var(--radius-xl)] border-l-[3px] px-4 py-[11px] text-[14px] font-semibold transition-colors ${
      active
        ? 'border-positive bg-positive/8 text-positive'
        : 'border-transparent text-text hover:bg-page'
    }`}
  >
...
const LogoutButton = ({ label }: { label: string }) => (
  <form action={logoutAction}>
    <button
      type="submit"
      aria-label={label}
      className="flex w-full items-center gap-3 rounded-[var(--radius-xl)] border-l-[3px] border-transparent px-4 py-[11px] text-left text-[14px] font-semibold text-error transition-colors hover:bg-error/8"
    >
...
const TopBarMobile = ({ onOpenDrawer }: { onOpenDrawer: () => void }) => {
  const tNav = useTranslations('nav')
  return (
    <header className="flex items-center gap-3 border-b border-border-soft bg-card px-4 py-3 md:hidden">
      <button
        type="button"
...
const Drawer = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <dialog
      ...
      className={[
        'm-0 h-full max-h-none w-72 max-w-[85vw] bg-card p-0 text-text md:hidden',
      ...
      <div className="flex h-full flex-col py-5">
        <div className="flex items-center justify-between px-4">
          ...
          <button
            ...
            className="ml-auto flex h-9 w-9 items-center justify-center rounded-[var(--radius-md)] text-text-soft transition-colors hover:bg-page hover:text-text"
          >
