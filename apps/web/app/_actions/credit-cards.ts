...
// ── 11.2: completeNovatoOnboarding ────────────────────────────────────────────
// Creates "Mi plata" (cash ARS) + "Mi tarjeta" (credit ARS, 2 estimated periods)
// Atomic: on any failure after the first insert, cleans up partial state.

export async function completeNovatoOnboarding(input: {
  close_date: string
}): Promise<ActionResult<never> & { cashAccountId?: string; cardId?: string }> {
  const userId = await getAuthenticatedUserId()
  const supabase = await createClient()
  const today = getTodayAR()
  const todayStr = formatDateISO(today)
  ...
  if (!input.close_date) {
    return { ok: false, formError: 'La fecha de cierre es obligatoria.' }
  }
  if (input.close_date < addDaysToISO(todayStr, -7)) {
    return {
      ok: false,
      formError: 'Tomá la fecha del próximo cierre que figura en tu resumen del banco.',
    }
  ...
  // 1. INSERT "Mi plata" (cash account with ARS)
  const { data: cashAccount, error: cashError } = await supabase
    .from('accounts')
    .insert({ user_id: userId, name: 'Mi plata', type: 'cash' })
    .select('id')
    .single()

  if (cashError || !cashAccount) {
    return { ok: false, formError: cashError?.message ?? 'Error al crear "Mi plata".' }
  }

  const { error: cashCurrencyError } = await supabase.from('account_currencies').insert({
  ...
  if (cashCurrencyError) {
    await supabase.from('accounts').delete().eq('id', cashAccount.id)
    return { ok: false, formError: cashCurrencyError.message }
  }

  // 2. INSERT "Mi tarjeta" (credit account, no institution/network)
  ...
  if (cardError || !card) {
    await supabase.from('accounts').delete().eq('id', cashAccount.id)
    return { ok: false, formError: cardError?.message ?? 'Error al crear "Mi tarjeta".' }
  }

  const { error: cardCurrencyError } = await supabase.from('account_currencies').insert({
    account_id: card.id,
    currency_code: 'ARS',
    initial_balance: 0,
    initial_balance_date: todayStr,
  ...
  if (cardCurrencyError) {
    await supabase.from('accounts').delete().eq('id', card.id)
    await supabase.from('accounts').delete().eq('id', cashAccount.id)
    return { ok: false, formError: cardCurrencyError.message }
  }

  // 3. INSERT 2 estimated card_periods
  const { close_date } = input
  const periodRows = [
    {
      account_id: card.id,
      start_date: addDaysToISO(close_date, -30),
  ...
  revalidatePath('/cards')
  revalidatePath('/accounts')
  return { ok: true, cashAccountId: cashAccount.id, cardId: card.id }
}
