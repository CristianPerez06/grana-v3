...
// filters. No React, no Supabase — testable in isolation. Mirror of
// apps/web/lib/cards/grouping.ts; keep the two in sync (same policy as the
// queries files).

import type { CreditCardSummary } from './queries'

/** Group urgency / row tone, matching the web `pillTone` rule. */
...
/** Per-card tone, same rule as the web `pillTone(period.alert, period.variant)`. */
