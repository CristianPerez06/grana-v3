...
// one with today's total, the USD line and "Dónde está" folded in; the light one
...
const Flow = ({
  ...
  <View className="flex-1 items-center">
    <View className="flex-row items-center gap-2">
      ...
      <Text className="text-[13px] font-bold text-text-muted">{label}</Text>
    </View>
    <MaskedAmount
      amount={ars}
      currency="ARS"
      className={`mt-2 text-[19px] font-extrabold ${amountClassName}`}
    />
    {usd !== 0 && (
      <MaskedAmount
        amount={usd}
...
export const BalanceCard = () => {
  ...
  const heroQuery = useDashboardHero()
