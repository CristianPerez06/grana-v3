...
#### Compartir — bloque desplegado
Handoff visual (2 rows de 38 px; atajos finales `Mitad · 70/30 · 75/25 · Todo suyo · Otro` + barra de reparto "Vos / [integrante del Hogar]"; "Otro" abre dos campos %) en [`compartir/README.md`](./compartir/README.md) + canvas [`compartir/compartir-bloque-final.html`](./compartir/compartir-bloque-final.html). ⚠️ **Reemplaza** lo que la propuesta OpenSpec tenía para Compartido (presets Vos/Mitad/El otro + "Otro %"): actualizar spec al implementar. Decidido con el PO: se quita "80/20" y "100%"→"Todo suyo" (5 chips en una fila). El estado `splitFirstPct` alcanza.

#### Recurrente — bloque desplegado
Handoff visual (2 rows de 38 px; chips de frecuencia + "Repetir hasta" opcional; modo personalizado transforma la row 2; línea gris con resumen en lenguaje natural) en [`recurrente/README.md`](./recurrente/README.md) + canvas [`recurrente/recurrente-bloque-final.html`](./recurrente/recurrente-bloque-final.html). **Anual/años se ocultan solo en mobile** (la funcionalidad sigue en backend/desktop; anual = "cada 12 meses"). El ícono de calendario del modo personalizado es "Repetir hasta" compactado (ver la nota del doc).
